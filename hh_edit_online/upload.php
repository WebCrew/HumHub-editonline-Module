...
if ($_FILES["file"]["error"] > 0)
    {
    echo "<div class='attention'>Return Code: " . $_FILES["file"]["error"] . "</div>";
    }
    if (file_exists($_REQUEST['folder'] . "/" . $_FILES["file"]["name"]))
      {
      echo "<div class='attention'>" . $_FILES["file"]["name"] . " already exists.</div>";
      }
    else
      {
      move_uploaded_file($_FILES["file"]["tmp_name"],
      $_REQUEST['folder'] . "/" . $_FILES["file"]["name"]);
	  
	  echo "<div class='attentiongreen'>File Uploaded</div>";
	  }
